...
var ZRComponent = (function () {
    ...
    ZRComponent.hideModalBox = function () {
        ZRComponent.enableScroll();
        if ($('.model-box-wrap').css('bottom') != '0px') {
            $('.model-box-wrap').css({top: "-300px"});
            $('.model-box-wrap').parent().fadeOut(1200, function () {
                if ($(this).data('removedialog') === true) {
                    $(this).remove();
                }
            });
        } else {
            var box = $('.model-box-wrap')[0];
            ZRComponent.popup.style.bottom = '';
            ZRComponent.popup.style.top = box.getBoundingClientRect().top + 'px';
            $('.model-box-wrap').css({top: -300});
            $('.model-box-wrap').parent().fadeOut(1200, function () {
                if ($(this).data('removedialog') === true) {
                    $(this).remove();
                }
    ...
    ZRComponent.generateModalBox = function (args, modalBox, ModalBoxType) {
        ...
        if (!$('.model-box-wrap').length) {
            ...
            ZRComponent.popup = document.getElementsByClassName('model-box-wrap')[0];
            ZRComponent.showModalBox(args.position);
        } else {
            //console.log(ZRComponent.ConstError.Error3);
            $('.model-box-wrap').parent().show();
            ZRComponent.showModalBox('middletop');
        }
    ...
    ZRComponent.showModalBox = function (position) {

        ZRComponent.disableScroll();
        $('.model-box-wrap').parent().fadeIn('fast');
        if (position == 'top') {
    ...
    ZRComponent.createAlert = function (args) {
        ...
        var removeFreeze = (args && args.hasOwnProperty('removeFreeze')) ? args.removeFreeze : true;
        ...
        var okButtonFun = (args && args.okbtnFn) ? args.okbtnFn :function () {ZRComponent.hideModalBox();};
        var okButtonArg = (args && args.okbtnArg) ? args.okbtnArg : undefined;
        var closeFun = (args && args.closeFn && typeof args.closeFn === 'function') ? args.closefn : function () {ZRComponent.hideModalBox();};
        ...
        args.position = (args && args.position && $.inArray(arg.position,positionArray)!=-1) ? args.position : ZRComponent.getDefaultModalBoxVal('position');

        var createElem = Utils.createHTML({"name":"div", "attr":{"class":needFreezeClass, "data-removefreeze":removeFreeze, "data-removedialog":removeDialog}, "child"://No I18N
            [{"name":"div","attr":{"class":modalClass, "id":modalId},"child":[{"name":"a", "attr":{"href":"javascript:void(0)", "class":"zrc-close-modal"}, "html":"&times;", "events":[{"name":"click", "fn": closeFun}]},//No I18N
                ...
                {"name":"div", "attr":{"class": btnAlignClass}, "child"://No I18N
                    [{"name":"input", "attr":{"type":"button", "class":okButtonType, "value":okButtonLabel}, "events":[{"name":"click", "fn": okButtonFun, "args":okButtonArg}]}]
                }]}]
        });
    ...
    ZRComponent.createConfirm = function (args) {
        ...
        var cancelButtonFun = (args && args.cancelButtonFun) ? args.cancelButtonFun : function () {ZRComponent.hideModalBox();};
        var cancelButtonArg = (args && args.cancelButtonArg) ? args.cancelButtonArg : undefined;
        ...
        var createElem = Utils.createHTML({"name":"div", "attr":{"class":needFreezeClass, "data-removefreeze":removeFreeze, "data-removedialog":removeDialog}, "child"://No I18N
            [{"name":"div","attr":{"class":modalClass, "id":modalId},"child":[{"name":"a", "attr":{"href":"javascript:void(0)", "class":"zrc-close-modal"}, "html":"&times;", "events":[{"name":"click", "fn": closeFun}]},//No I18N
                ...
                {"name":"div", "attr":{"class": btnAlignClass}, "child"://No I18N
                    [
                        {"name":"input", "attr":{"type":"button", "class":confirmButtonClass, "value":confirmButtonLabel}, "events":[{"name":"click", "fn": confirmButtonFun, "args":confirmButtonArg}]},
                        {"name":"input", "attr":{"type":"button", "class":cancelButtonClass+" marL15", "value":cancelButtonLabel}, "events":[{"name":"click", "fn": cancelButtonFun, "args":cancelButtonArg}]}
                    ]
                }]}]
        });
